...
def generate_circle_waypoints():
    ...
    center_x = w// 2        # 1200
    center_y = h // 2       # 720
    radius = 350                 # Adjust size if needed
    num_points = 10             # Smooth circle
...
def generate_I_Yold():
    ...
    min_x, max_x = 428, 1578
    min_y, max_y = 401, 1070
    ...
    for x in np.linspace(left, right, 10):
        waypoints.append((int(x), int(y_pos)))
...
def generate_I_Y():
    ...
    num_points = 10  # how many points you want in between

    for i in range(1, num_points + 1):
        t = i / (num_points + 1)  # interpolation factor (0 → 1)
...
def generate_O_Y():
    ...
    for theta in np.linspace(0.65, 2*np.pi, 25):
        x = center_x + radius_y * np.cos(theta)
        y = center_y + radius_x * np.sin(theta)
        waypoints.append((int(x), int(y)))
def generate_R_Y():
    waypoints.clear()
    ...
    # Horizontal spine
    for x in np.linspace(right, left, 5):
        waypoints.append((int(x), int(bottom+0.01*x)))

    # Side curve
    radius_x = (right - mid_x) // 2
    radius_y = 300
    center_x = mid_x - 280
    center_y = top + radius_y

    for theta in np.linspace(-np.pi, -np.pi/6, 10):
        x = center_x + radius_x * np.cos(theta)
        y = center_y + radius_y * np.sin(theta)
        waypoints.append((int(x), int(y)))

    for theta in np.linspace(5*np.pi/6, np.pi/3, 5):
        x = 180+mid_x + 0.8*radius_x * np.cos(theta)
        y = -250+center_y + 0.8*radius_y * np.sin(theta)
        waypoints.append((int(x), int(y)))
    # for theta in np.linspace(1.2*np.pi/2, np.pi/2, 10):
    #     x = 200+mid_x + radius_x * np.cos(theta)
    #     y = -300+center_y + radius_y * np.sin(theta)
    #     waypoints.append((int(x), int(y)))
    stx = x
    sty = y
    # Diagonal leg
    for t in np.linspace(0.1, 0.25, 3):
        x = stx + t * ( mid_x)
        y = sty - t * ( center_y)
        waypoints.append((int(x), int(y)))
def generate_S_Y():
    ...
    for t in np.linspace(0, 1.3, 25):
        x = left + t * (width)
        y = center_y + amplitude * np.sin( 2*np.pi * t - np.pi/4) / 2
        waypoints.append((int(x), int(y)))

def generate_sine_wave_waypoints():
    waypoints.clear()

    amplitude = 300
    frequency = 2*np.pi / 800
    y_offset = h // 2
    num_points = 20

    # Keep inside screen range
    x = np.linspace(w//2 - 800, w//2 + 700, num_points)
...
if __name__ == '__main__':
    try:
        ...
        timestamped_filename_video = f"/home/fishcam_ros_ws/src/fish_pc/2D_tracking_results/RPID_vel2.5_run10.6kp0.5kd_veltest{current_datetime.strftime('%Y%m%d_%H%M')}.avi"
        # Define the codec and create VideoWriter object
        ...
        # generate_sine_wave_waypoints()
        # generate_R_waypoints()
        generate_R_Y()
