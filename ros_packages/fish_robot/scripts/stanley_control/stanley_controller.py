...
class Fish_PurePursuitController:
    ...
    def motor_pub(self):
        rate = rospy.Rate(20)  # publish at 20 Hz (adjust)
        while not rospy.is_shutdown():
            # Publish encoder data
            e_vel = self.module.get("brushless_drive", "obs_velocity")
            e_pos =  self.module.get("brushless_drive","obs_angle")
            voltage = self.module.get("power_monitor", "volts") #tm.Vbus.magnitude
            current = self.module.get("power_monitor","amps") #tm.Iq["estimate"].magnitude
            ...
            if all(isinstance(x, float) for x in [e_vel, e_pos,voltage, current, power]):
                ...
                enc_msg.encoder_vel = e_vel   # Using x to store velocity
                enc_msg.encoder_pos = e_pos  # Using x to store velocity
                enc_msg.voltage = voltage #tm.Vbus.magnitude
                enc_msg.current = current #tm.Iq["estimate"].magnitude
                enc_msg.power = power
    ...
    def update_control(self):
        while not rospy.is_shutdown():
            ...
            if self.pose is not None and self.path:
                ...
                self.servo_angle  = (np.rad2deg(error_yaw) + np.rad2deg(math.atan2(self.Ks*self.e_fa,self.pose[3])) )  # stanley control law
                self.prev_error_yaw = error_yaw
                self.set_servo_angle()
                
                msg_pp = purepursuit_control()
                msg_pp.header.frame_id = "stanley_control"
                msg_pp.header.stamp = rospy.Time.now()
                msg_pp.goal_x = goal[0]
                msg_pp.goal_y = goal[1]
                msg_pp.Ld = self.Ld
                msg_pp.error_yaw = error_yaw
                msg_pp.yaw_Kp = self.Ks
                msg_pp.yaw_Ki = self.theta_p 
                ...
                if self.vel_cntrl == True:
                    ...
                    self.vI = min(max(self.vI + error_vel*self.dt, 0), 750) 
...
if __name__ == '__main__':
    try:
        ...
        motor_pub_thread = threading.Thread(target=controller.motor_pub)
        motor_pub_thread.daemon = True
        motor_pub_thread.start()
