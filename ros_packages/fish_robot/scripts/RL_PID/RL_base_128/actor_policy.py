...
class ActorNumpy:
    ...
    def forward(self, obs):
        ...
        Ki_a = self.to_range_tanh(raw[1], 0, 0.05)
        Kd_a = self.to_range_tanh(raw[2], 0, 0.05)

        # Speed PID (throttle)
        Kp_s = self.to_range_tanh(raw[3], 0.0, 2.0)
        Ki_s = self.to_range_tanh(raw[5], 0.0, 0.5)
        Kd_s = self.to_range_tanh(raw[4], 0.0, 0.5)
