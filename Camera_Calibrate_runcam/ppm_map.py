...
# Prepare CSV file
csv_filename = "ppm_measurements.csv"
# with open(csv_filename, mode="w", newline="") as f:
#     writer = csv.writer(f)
#     writer.writerow(["center_x", "center_y", "ppm_w", "ppm_h", "ppm_mean"])
    ...
    if not contours:
        ...
        if frame_count > frame_initialize:
            ...
            if err < 1e-4:
                ...
                pixels_per_m_f = np.mean(Cum_pixels_per_m)#sum(Cum_pixels_per_m*(object_size/sum(object_size)))
...
ppm_w, ppm_h = Cum_pixels_per_m
ppm_mean = np.mean(Cum_pixels_per_m)
